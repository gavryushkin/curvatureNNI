...
%DIF DEL curvatureNNI_JMB_original_submission.tex   Fri Apr 21 10:30:32 2017
%DIF ADD curvatureNNI.tex                           Fri Apr 21 11:02:36 2017
...
\begin{figure}[ht]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.8\textwidth]{DtT.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.8\textwidth]{DtT_with_rank_move_short}
...
Assuming that the length of every event interval is either $1$ or $2$, the outer trees are all possible neighbors in $\dtt$ of the tree in the middle \DIFdelbeginFL \DIFdelFL{corresponding to }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{obtained by moves performed on intervals $I$ and $J$, where the trees on the right are obtained by moves performed on }\DIFaddendFL interval $I$ \DIFaddbeginFL \DIFaddFL{and on the left---on $J$}\DIFaddendFL .}
\label{DtT.pdf}
\end{figure}
...
\proof
We describe the proof in detail for $\rnniu$ \DIFdelbegin \DIFdel{, and include proof sketches }\DIFdelend \DIFaddbegin \DIFadd{and then explain how to modify the proof }\DIFaddend for the other three graphs\DIFdelbegin \DIFdel{in Section~\ref{secAppendix}.\ Appendix}\DIFdelend .
...
\DIFaddbegin \DIFadd{Here, for every edge in the tree we distinguish between its two ends and refer to them as half-edges---this is necessary to be able to substitute one subgraph by another in a unique way---as in }\autocite{Sleator1992-bp}\DIFadd{.
...
\DIFaddbegin \DIFadd{See }\autocite{Sleator1992-bp} \DIFadd{for precise definitions and details.
}\DIFaddend 
...
\DIFdelbegin \DIFdel{See }\DIFdelend \DIFaddbegin \DIFadd{Specifically, Theorem~2.3 from }\DIFaddend \autocite{Sleator1992-bp}\DIFdelbegin \DIFdel{for precise definitions and details.
}\DIFdelend \DIFaddbegin \DIFadd{, adapted to trees, claims the following:
}
...
\DIFdelbegin \DIFdel{See Section~\ref{secAppendix}.
...
\DIFdelbegin \DIFdel{The proof of this theorem can be found in Section~\ref{secAppendix}.
\ Appendix}\DIFdelend \DIFaddbegin \proof
...
Now, Conjecture~\ref{conjSplit} implies that no minimal length path from $x$ to $y$ will replace a bipartition that is common to $x$ and $y$}\DIFaddend .

\DIFaddbegin \DIFadd{We are now ready to bound the sizes of each partition $I$, $R$, and $L$.
...
\DIFaddend We conclude by noting that most of the results in this paper can be generalized to the class of sampled ancestor trees \autocite{Gavryushkina2014-xd}\DIFaddbegin \DIFadd{, where $\dtt$ trees are enriched by an extra event called }\emph{\DIFadd{sampled ancestor}}\DIFadd{---an internal node marked by a taxon with exactly one child}\DIFaddend .
