...
A Markov chain (often a random walk in our case) is defined on a metric space by its proposal mechanism, which is a functional $m$ that maps points of the space to the set of measures on this space. \todo{Note that Ollivier has a specific definition of the term "random walk" that I think would be good to use here as well.}
...
Given a metric space\footnote{Since all metric spaces considered in this paper are finite, they can be seen as graphs by making points at distance one adjacent.} \todo{Er, not all finite metric spaces can be turned into such a graph}
...
\todo{Wait, I thought you just said that it was NP-complete.}
