...
Hence, instead of considering the space in full, we approximate the space by a sequence of finite spaces and study the geometry and convergence of random walks over these approximations.
...
Thus we have defined a graph, which we denote by $\mdts$.
...
We denote NNI space on (ultrametric) rooted tree topologies by $\nni$ ($\nniu$).
...
Then
\[2(n-1) \leq \deg(T) \leq 5n-6\] in $\mdts$ space,
\[\deg(T) = 2(n-2)\] in $\nni$ space, and
...
\proof
The lower bound in $\mdts$ space is attained by any tree with all intercoalescent intervals being long.
...
The upper bound is obtained in the same way as for $\mdts$ space.
\endproof

We need the following obvious corollaries for our analysis of curvature.
...
\proof
Follows from Lemma~\ref{neighBound} using a similar observation as in the proof of Corollary~\ref{degreeBounds}.
\endproof
...
Thus, the upper bound of asymptotic curvature of $\nni$, $\rnni$, and $\mdts$ spaces coincides with the coarse curvatures of the spaces.
