...
Non-ranked trees have a symmetry associated with every internal node.
...
Then $T = T_\sigma$ if and only if $\sigma$ is a transposition of a pair of taxa that form a
cherry\footnote{A {\em cherry} is a pair of taxa adjacent to a common internal node in the tree.}
in $T$.
\end{proposition}

\proof
The sufficiency if obvious.
For necessity, assume that $\sigma$ transpose taxa $i,j$ such that $i,j$ is not a cherry in $T$.
This implies that $i$ and $j$ have different parents.
Hence $T \ne T_\sigma$ because the ranks of the parents of $i$ and $j$ are different in $T$ and $T_\sigma$.
\endproof
