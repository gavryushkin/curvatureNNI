...
In this paper we study Ricci-Ollivier curvature~\cite{Ollivier2009-cj} of Markov chains over phylogenetic trees~\cite{Semple2003-nj}\todo{CW: Depending on venue, this sentence should probably be expanded to a paragraph motivating phylogenies, particularly time-trees.}.
...
In the rest of the paper, we consider a number of Markov chains that arise in phylogenetic Bayesian inference and investigate the curvature of those chains with the eventual goal of the efficiency properties of the corresponding inference methods\todo{CW: I suggest summarizing the major results here, particularly any that only make sense after curvature is defined and thus don't belong in the abstract. This could wait until the analytical results are ready though.}.
...
Although the investigation of curvature and convergence properties of random walks over the space of time-trees is interesting and important\todo{CW: Why? this needs to be justified either here or in the introduction}, the geometry necessary for these investigations is yet to be developed to the necessary level~\cite{Gavryushkin2014-bw}.
...
For ultrametric trees, this sum would be $n-1 + 2(n-2) = 3n-5$\todo{CW: Why? please add a brief breakdown for these terms as with the other spaces.}.
...
The lower bound in $\rnni$ space is attained by the caterpillar-tree where taxa get ranks $1, 2, 4, 6, \ldots$ and internal nodes get ranks $3, 5, 7, \ldots$. \todo{CW: didn't we define ranks to start from 0? Please standardize on one or the other. Also, please add the end values of these sequences after you do so.}
...
\begin{corollary}\label{degreeBounds}
	The following are satisfied in $\mdts$ space for every pair of trees $T$ and $R$.\todo{CW: Please standardize the terms here. Do we want to use e.g. $2n-4$ or $2(n-2)$. Some of these are different from those in the lemma.}
\begin{align*}
