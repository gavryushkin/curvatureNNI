...
Then $\Delta(\rnniu) \le \frac34 (n-1)^2 + \frac14 (n-1)$.
\end{lemma}

\proof
Let $T$ and $R$ be trees in $\rnniu$.
We show that there exists a path between them of length $\frac34 n^2 + \frac14 n$ or shorter.
We denote that taxa on one side from the root of $T$ by $A$ and the rest of taxa by $B$.
...
We construct the path from $T$ to $R$ proceeding in the following seven steps:
\begin{enumerate}[(1)]
