...
%EM I would prefer the following title:
%EM The combinatorics of discrete time-trees: theory and open problems
\title{Combinatorics of discrete time-trees: algorithmic insights and open problems}
...
We settle many of these questions by applying the concept of graph grammars introduced by Sleator, Tarjan, and Thurston.
...
%EM given that this is an important object in the paper, perhaps a slightly tighter definition is merited? For example, it's not until the end of this paragraph when we learn that the total order enforced by the divergence times must be compatible with the partial order given by the tree structure.
...
%EM Re "In other words" this has lost its continuity by the insertion of the metric space thing, which in any case I don't think deserves its own paragraph.
...
\begin{figure}[ht]
\centering
%EM are the "a" and "b" labels of this figure used in the text?
...
\end{corollary}
%TODO: Drop this if we drop split theorem.
...
We use the following algorithm to compute the $\rnni$ graph on $n$ leaves, which is a natural extension of an algorithm from \textcite{Whidden2015-es}.
...
\todo[inline]{Show that there are many unsolved problems in the area that other researchers may be interested in working on.
Add a "future directions" section, with a mention in the abstract/introduction.}
...
%EM Hm... regarding this last statement, couldn't some graphs go to zero more quickly than others?
