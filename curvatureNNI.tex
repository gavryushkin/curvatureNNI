...
Hence, for each of the possible discretization of the time-tree space, a natural question is the following.

\begin{problem}
What is the complexity of computing the distance between two trees in discrete time-tree space?
\end{problem}

Although we do not know the complexity of finding the distance between two caterpillar trees in $\nni$ space, the following example suggests that the problem has higher complexity in $\nni$ than in $\rnni$.

Let $T$ be the ultrametric tree $(((((((1, 2), 3), 4), 5), 6), 7), 8)$ and $R$ be the ultrametric tree $((((((1, 2), 5), 6), 2), 3), 4), 7)$.
Then a shortest $\nni$ path is given by first making a cherry $(3,4)$, then moving the cherry up to the split $14567 \mid 8$, and then resolving the cherry back.
While in $\rnniu$ this path is not shortest, and a shortest path would move the parents of $3$ and $4$ independently.
\todo{Make this an $\backslash$example}
...
\begin{proposition}
Ranked trees don't have non-trivial symmetries, hence tanglegrams don't help.
\todo{Make this formal}
\end{proposition}

These observations suggest that $\nni$ space is geometrically and algorithmically different from the other four spaces.

We continue with the following bounds on the sizes of one-neighborhoods of various discretization of the space of time-trees, which are one of the most important tools for estimating the curvature of various random walks later in the paper.
...
Clearly, $\lim_n \frac{|\partial S|}{|S|} = 0$ for all graphs under consideration.
\endproof
