...
Note also that $\mathrm{mrca}(C,E)$ in $T$ is not a node of such sort, so the tree obtained from $T$ by cutting the edge adjacent to $C$ and regrafting it to the edge between the aunt of $\mathrm{mrca}(C,E)$ and the root does not give an NNI neighbour of $T$ in this sense.
