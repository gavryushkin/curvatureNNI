...
Then $\Delta(\rnniu) \le (n-1)^2 + \frac14 (n-1)$.
\end{lemma}

\proof
Let $T$ and $R$ be trees in $\rnniu$.
We show that there exists a path between them of length $n^2 + \frac14 n$ or shorter.
...
\item Move all the nodes adjacent to taxa in $A_1$ on top of all nodes in $T$, apart from the root and the nodes adjacent to taxa in $A_2$.
The tree restricted to $A_1$ is then a caterpillar tree.
This can be done in $2m^2$ moves.
(We refer to this type of action at to ``move $A_1$ in between $A_2$ and $B$'')

\item Move $B_1$ in between $A_1$ and $B_2$.
...
\item We can assume that there exist trees $\operatorname{join}(T_A, T_B)$ and $\operatorname{join}(R_A, R_B)$ on the shortest path from $T$ to $R$ such that
\[
