...
In short, the {\em discrete time-tree space,} or $\dts$ space, is the $\tau$-space\footnote{In this discrete setting, $\tau$- and $\mathrm{t}$-spaces from~\cite{Gavryushkin2014-bw} coincide.} defined in~\cite{Gavryushkin2014-bw}, where the intercoalescent intervals are allowed to take only two length values, $1$ and $2$.
...
The node of the least rank is seen as being at the present, so the root is the oldest node of the tree of the highest rank.
...
In particular, $\mathrm{mrca}(D,E)$ in tree $T$ on Figure~\ref{dts_neighbours.pdf} gives an example of such a node $a$.
Note also that $\mathrm{mrca}(C,E)$ in $T$ is not a node of this sort, so the tree obtained from $T$ by cutting the edge adjacent to $C$ and regrafting it to the aunt of $\mathrm{mrca}(C,E)$ does not give an NNI neighbour of $T$ in this sense.
...
See Figure~\ref{dts_neighbours.pdf} \todo{Draw the picture.} for an example of a pair of trees $T$ and $R$ at $\dts$ distance $3$, where one has to apply all of the three possible cases from the definition above to obtain a path from $T$ to $R$ in $\dts$.
...
\[\deg(T) = 2(n-2)\] in $\nni$ space, and
\[n-1\leq \deg(T)\leq3(n-1)\] \todo{should this be $3n-4$ instead?} in $\rnni$ space.
\end{lemma}
