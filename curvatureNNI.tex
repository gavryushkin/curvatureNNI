...
\begin{lemma}\label{splitCatLemma}
Let $T$ and $R$ be two caterpillar-trees sharing a split of taxa $A|B$.
Then every tree on every shortest $\rnniu$ path between $T$ and $R$ has split $A|B$.
\end{lemma}

\proof
Assume the contrary and let $T$, $R$, $A|B$, and $T = T_0, \ldots, T_r = R$ be the trees, split, and path that provide a counter-example.
Let also the set $A$ be $\{0,\ldots,k\}$, $B$ be $\{k+1,\ldots,n\}$, $T$ be $(\ldots(0,1),\ldots,),n)$, and $R$ be $(\ldots(\sigma 0,\sigma 1),\ldots,),\sigma n)$, where $\sigma$ is a permutation of $(0,\ldots,n)$.
Since $T$ and $R$ share the split $A|B$, $\sigma A = A$ and $\sigma B = B$.

We say that two taxa in a tree are {\em adjacent} if they have either identical or adjacent parents.

We can assume that there exist an $i_1$ such that taxon $1$ is adjacent to a taxon $x_1$ from $B$ in $T_{i_1}$, an $i_2$ such that taxon $2$ is adjacent to a taxon $x_2 \ne x_1$ from $B$ in $T_{i_2}$, $\ldots,$ and an $i_k$ such that taxon $k$ is adjacent to a taxon $x_k \notin \{x_1, \ldots, x_{k-1}\}$ in $T_{i_k}$.
Furthermore, we can assume that $B = \{x_1, \ldots, x_k\}$.

Indeed,\todo{}

Moreover, we can rearrange the path from $T$ to $R$ so that $i_1 = \ldots = i_k$.
Denote this number by $i_0$ and let $i$ be the smallest and $j$ be the greatest among such $i_0$'s.

Clearly,
\[
d(T, T_i) \ge 1 + \ldots + k = \frac 12 k(k+1),
\]
and
\[
d(T_j, R) \ge \frac 12 k(k+1).
\]

We claim that $d(T_i, T_j) \ge d(T_A, R_A)$.
Indeed,\todo{}

Corollary~\ref{diameterUpperBoundCoro} implies that $d(T_B, R_B) < k(k+1)$.
Hence, $r \ge k(k+1) + d(T_A, R_A) > d(T_B, R_B) + d(T_A, R_A)$.
This however is a contradiction.
\endproof

\begin{corollary}
The same is true in $\mdts$, $\mdtsu$, and $\rnni$.
\end{corollary}
