...
A \emph{discrete time-tree} is a tree such that all its nodes are assigned distinct times from the set of non-negative integers such that every node has a lesser time than its parent.
Note that this implies that for every pair of nodes $x,y$ if the shortest path from the root to $x$ passes $y$ than the time of $y$ is greater than the time of $x$.
The \emph{rank} of a node is the number of nodes in the tree with strictly smaller~\todo{lesser?} time.
