...
\todo{Do they?}
in phylogenetic Bayesian inference and investigate the curvature of those chains with the eventual goal of the efficiency properties of the corresponding inference methods.
...
Furthermore, as the NNI space of unrooted tree topologies with $n-1$ leaves is exactly the same as the NNI space of rooted tree topologies with $n$ leaves, all our result immediately imply their counterparts in unrooted NNI space.
...
We denote NNI space on rooted tree topologies by $\nni$, on ranked tree topologies by $\rnni$,
\todo{TODO: Give proper definitions.}
and on $\dts$ trees with $m$ branch lengths by $\mdts$.
...
Then \[2(n-1)\leq \deg(T)\leq5(n-1)\] in $\dts$ or $\mdts$ space, \[\deg(T) = 2(n-2)\] in $\nni$ space, and \[n-1\leq \deg(T)\leq3(n-1)\] in $\rnni$ space.
\end{lemma}
...
Since every NNI move results in two neighbours, the equality $\deg(T) = 2(n-2)$ follows, because no NNI is possible for the cherry branches.
...
For $\nni$ space, Lemma~\ref{neighBound} implies that are $deg(T)-deg(R) = 0$ and $\frac{\deg(T)}{deg(R)} = 1$.
...
\frac{1}{2(n-1)}
\]
The upper bound of asymptotic curvature of $\nni$ and $\rnni$ spaces with uniform $p$-lazy random walk coincides with the (plain) curvatures of the spaces with uniform random walk as well. 
