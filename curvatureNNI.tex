...
1.\ a section on 1-neighborhood sizes, 2.\ a section on larger neighborhood sizes and diameter, 3.\ degreeBounds, intersecNeighb, and max good neighbors,
\iftoggle{curvatureON}{
4.\ curvature.
}{}
...
\begin{itemize}
\item All nodes have degree $1$, $2$, or $3$.
\item There exists exactly one node of degree $2$. This node is called the root of the tree.
\item All nodes of degree $1$ are marked by different constants. These nodes are called leaves.
\end{itemize}
...
By a {\em time-tree} we mean a rooted phylogenetic tree with an absolute time associated with every node of the tree: for internal nodes these are (typically estimated) divergence times and external nodes are (typically known) sampling times.
By ``absolute'' we clarify that these are not relative times, but rather actual times that can be put on a calendar.

A {\em discrete time-tree} is a tree such that all its nodes are assigned distinct ranks from the set of non-negative integers with the property that for every pair of nodes $x,y$ if the shortest path from the root to $x$ passes $y$ than the rank of $y$ is greater than the rank of $x$.
We say that a pair of nodes $x,y$ of a discrete time-tree is an {\em intercoalescent interval} if there exists no node $z$ such that the rank of $z$ is between the ranks of $x$ and $y$.
We call the difference between the ranks of $x$ and $y$ the {\em length} of the interval $x,y$.
We identify two discrete time-trees on the same set of leaf-labels 

Figure~\ref{T5.pdf} provides an example of a discrete time-tree.
...
$\nni$ space is a graph with the vertices being all trees on $n$ leaves.
...
For $m > 0$, level $m$ of the hierarchy is the following graph $\dtt_m$.
The set of nodes of the graph is the set of discrete time-trees on $n$ leaves such that every intercoalescent interval has its length not greater than $m$.
...
We say in the latter case that $R$ is obtained from $T$ by an $\nni$ move performed on interval $I$.

Note that $\dtt_0$ can be obtained from $\dtt_1$ by ``forgetting'' ranks and $\dtt_1$---from $\dtt_m$ by forgetting lengths.

This hierarchy can be seen as a set of discrete refinements of the full space of phylogenetic time-tree, where intercoalescent intervals can take an arbitrary real value \autocite{Gavryushkin2014-bw}.
...
\label{T5.pdf}
\caption{Discretized time-tree from $\dtt_2$ on $6$ leaves.
Intercoalescent intervals can take only finitely many values (two in this example).}
\end{figure}

\todo[inline]{It would seem tidiest to me to just define discrete time trees as trees where the ranks can take values in the positive integers. The mdts is the subset of such trees where the interval between ranks is at most m.}
...
\todo[inline]{Would be fun to have a diagram showing all of the neighbors of a tree obtained by modifying a given interval.}
Two trees $T$ and $R$ from $\dtt$ are adjacent if and only if precisely one of the following conditions is satisfied.
\begin{enumerate}[(1)]
...
In other words, by going from one tree to an adjacent tree in the $\dtt$ graph we can either change the length of one intercoalescent interval by one unit, swap the rank of two nodes, or send the length of an intercoalescent interval of minimal length down to zero and then resolve the multifurcation to either of the two possible topologies.
In the latter case, the new interval is of minimal length.
See Figure~\ref{dts_neighbors.pdf} for an example of a pair of trees $T,R$ at $\dtt$ distance $3$, where one has to apply all of the three possible cases from the definition above to obtain a path from $T$ to $R$ in $\dtt$.

\begin{figure}[ht]
\centering
\includegraphics[width=0.7\textwidth]{dts_neighbors.pdf}
\label{dts_neighbors.pdf}
\caption{Trees $T$ and $R$ are at $\dtt$ distance $3$.
To move from $T$ to $R$ in $\dtt$, one must decrease the length of interval $I$, swap the ranks of nodes $A$ and $C$ bounding the interval $J$, and perform an NNI move on interval $K$, resolving nodes $C$ and $D$.}
\end{figure}

Thus we have defined a graph, which we denote by $\dtt$ for $m \geq 2$.
Since a graph can also be seen as a metric space where the distance is given by the length of a shortest path, we will refer to $\dtt$ as both.

One might suggest that geometrically and graph-theoretically $\dtt$ is similar to NNI space on (rooted) tree topologies, to ranked NNI space, and to $\dtt$ space on ultrametric trees. \todo{This sentence is hard to parse!}
That is so indeed.
As we shall see, the results and proofs in this paper can be straightforwardly adapted to the spaces mentioned.
\todo[inline]{Do you prove new results about (unranked) NNI? If so, would be great to point that out in the intro.}
...
We will provide complete proofs for $\dtt$ space and draw the corresponding results for the other three treespaces simultaneously.

The $\dtt$ space on ultrametric trees is denoted by $\dttu$.
The $\dtt$ space with all intercoalescent intervals of length one is denoted by $\rnni$ and called {\em ranked NNI} space; if this space is defined on ultrametric trees we will denote it $\rnniu$.
