...
\item A more useful version of the previous one:\\
The number of trees at the distance up-to $r$ from any given tree is at most $3^{f(n)}2^{g(r)}$, where both $f$ and $g$ are linear.\\
Can this be derived directly from Lemma~1 in \autocite{li1996some}, or we have to encode ranks of the nodes into the graph structure of the tree and apply Sleator--Tarjan--Thurston?\\
The latter gives $3^{2(n-2)} 2^{4r}$ if we use an extra vertex for every internal node of the tree to encode its rank---see Figure~\ref{rankEncoding.pdf}. Is this an overkill?
\begin{figure}
\centering
\includegraphics[width=.7\textwidth]{rankEncoding.pdf}
\caption{Ranked tree represented as graph.}
\label{rankEncoding.pdf}
\end{figure}
