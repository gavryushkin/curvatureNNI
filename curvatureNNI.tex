...
\proof
We describe the proof in detail for $\rnniu$, and include proof sketches the other three graphs in Section~\ref{secAppendix}.\ Appendix.
...
See Section~\ref{secAppendix}.\ Appendix for details.
\endproof
...
Indeed, since sorting a caterpillar tree of size $k$ in $\rnni$ takes shorter than merging two such caterpillar trees and then separating them, the counter-example from \autocite{li1996some} does not apply in $\rnni$.
Hence, if a counter-example to the Split Theorem exists in $\rnni$ then the trees that deliver the counter-example must be more complex than those obtained by ``gluing together'' two caterpillar trees.
...
In this section, we bound the number of neighbors of a tree $x$ in the discrete time-tree graphs that are closer than $x$ to a given tree $y$, \textbf{under the assumption that Conjecture~\ref{conjSplit} holds}.\footnote{
...
The proof of this theorem can be found is Section~\ref{secAppendix}.\ Appendix.
...
\proof[Proof of Theorem~\ref{neighSizeTh} for $\rnni$, $\dttu$, and $\dtt$]
Following the terminology and notations from the $\rnniu$ case, we proceed as follows.
