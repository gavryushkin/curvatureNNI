...
Although Markov chain Monte Carlo (MCMC) is guaranteed to sample from the true posterior given an infinite run time, it is important to understand mixing properties of the chain, which determine sampling properties for a finite time run.\todo{CW: Figure 1 doesn't match the intro, where we define a time tree as ``a rooted phylogenetic tree with all internal nodes equipped with absolute divergence dates''. I suggest either adding a new figure here and moving this one to the technical introduction (where it is first referenced) or adding a time axis to the figure. If we leave it here, then the caption needs to be expanded as well.}
