...
%CW this is rather vague, can we call out a few specific results? (Maybe EM's suggestion from below?)
...
We also design an efficient algorithm for constructing these graphs.
\iftoggle{curvatureON}{
We also investigate the Ricci-Ollivier curvature of basic random walks on the space.
}{}

%CW: this is also rather vague, can we make it more direct and specific?
...
% As with the abstract, this paragraph is quite vague, probably as a result of dropping curvature. We should discuss specific results here.
...
%EM If we want to include this "tail" comment I suggest that it be clarified. IIUC there are lots of tails, right?
...
% Along the lines of EM's comment below, we should note that we prove here that these spaces have linear-size neighborhoods which are efficiently enumerable, and important property for efficient phylogenetics. We could also note that these insights are important for developing tree proposals, either here or in the abstract and/or intro
We continue with the following bounds on the sizes of $1$-neighborhoods of various discretizations of the space of time-trees (see Section~\ref{secAppendix} for all omitted proofs).
\iftoggle{curvatureON}{
...
%EM I suggest adding some text here that says basically "here's the big one!" There are lots of lemmas and things in this section, some of which are simple to prove, and it's easy to miss that this one is extra exciting.
% In fact, I think that one could make an argument for putting a proof sketch for this theorem in, and moving the proof of the diameter upper bound to the appendix.
%CW I second this idea and think we should also prominently feature this theorem in the abstract and at the end of the introduction.
...
\textbf{Computing the distance.} We now bound the number of neighbors of a tree $x$ in the discrete time-tree graphs that are closer than $x$ to a given tree $y$, \textbf{under the assumption that Conjecture~\ref{conjSplit} holds}.\footnote{
...
We conclude by noting that most of the results in this paper can be generalized to the class of sampled ancestor trees \autocite{Gavryushkina2014-xd}.
...
\proof[Proof on Lemma~\ref{neighBound}]
...
\proof[Proof of Theorem~\ref{neighSizeTh}]
...
\proof[Proof of Theorem~\ref{max_good_neighbors}]
