...
Posterior distributions on both time-trees and classical trees are estimated using Markov chain Monte Carlo \autocite{Mau1997-sq,Yang1997-gv,Drummond2002}, but with different transition kernels.
...
This paper builds a foundation for understanding mixing properties of MCMC on time trees.
...
Common moves include \emph{subtree prune and regraft} (SPR) moves, which cut a subtree off and reattach it at another location, and the subset of SPR moves called \emph{nearest-neighbor interchange} ($\nni$) moves.
...
Up to now, the only discretization of time-tree space that has been studied is that of ranked trees \autocite{Ford2009-qi,Lambert2013-mr}, and that was not from a graph-theoretic perspective.

In this paper we initiate the mathematical study of discretized time-tree graphs, obtain basic geometric and graph-theoretic results,
\iftoggle{curvatureON}{
...
(1) all nodes have degree $1$, $2$, or $3$; (2) there exists exactly one node of degree $2$, this node is called the \emph{root} of the tree; (3) all nodes of degree $1$ are labeled by distinct identifiers, these nodes are called \emph{leaves} or \emph{taxa}.
...
%EM I think it would be worth clarifying that the "intercoalescent interval" is a slight misnomer for the non-ultrametric case, because there are intercoalescent intervals between the appearance of taxa, although no coalescence has happened there. Would "event interval" be more appropriate, or is it too late to change?
%EM Also, I suggest pointing out that taxa can be younger than coalescence nodes, because none of the examples show this explicitly.
...
%EM Rather than "suppressing", I suggest "removing" as I think it's more clear. Or did you mean "shrinking" the edge to zero length, making the corresponding identifications?
...
%EM It's not so obvious from this following sentence if dtt is the union of the dtt_m 's or if it's a generic name for any one of them.
For simplicity, we use the general identifier $\dtt$ to refer to $\dtt_m$ with an arbitrary $m \geq 0$.
...
%EM Suggest changing to RNNI, and specifically calling out that this is not rooted NNI.
...
%EM seems a little funny to put Semple2003-nj as a reference in the proof like this, as they just collected results. I suggest just saying that proofs can be found in that book after the statement of the lemma.
...
We continue with the following bounds on the sizes of one-neighborhoods of various discretizations of the space of time-trees.
\iftoggle{curvatureON}{
...
\proof
%EM Doesn't this need to be "of length $m > 1$"?
...
%EM IIUC, "running tree" is your terminology. Please define.
...
%EM Given that the enumeration is actually pretty simple given a grammar, I suggest a sentence something like "The number of possible productions to the power of the encoding length for a given neighborhood radius gives a simple upper bound on the number of trees in a neighborhood."
...
%EM Sorry, but it's confusing here how 1,2,3 are used here twice for two distinct purposes. alpha, beta, gamma?
\begin{figure}
\centering
...
%EM This is their Section 3.2, right? Perhaps also 3.4? Would be nice to be specific if possible.
This bound can be improved following the techniques described in \autocite{Sleator1992-bp} after their Theorem~2.3 by bounding the number of possible productions available at a given time by a number smaller than their total.
...
%EM Note to self: read after Alex revises after Chris' comments.
For $\rnni$, we extend graph grammar $\Gamma$ by two productions, namely, one for the move when two taxa swap their ranks and one for the move when a taxon and an internal node swap their ranks.
...
For $\dttu$, we have to extend grammar $\Gamma$ by two productions, namely, one for increasing the length of the brunch between two nodes in the production and one for decreasing.
This increases the number of necessary node labels by one.
Indeed, if a production has a long brunch on the left side, three labels are enough to indicate whether the branch length increases or decreases; if a production has a short branch on the left side three labels are needed for the top node to indicate the type of the move that has to be applied: length increase move or one of the two possible $\nni$ moves.
...
Theorem~\ref{neighSizeTh} provides a method for estimating the diameter of the graph from below. %EM which results in a bound of ...
...
%EM I suggest a different letter than m, because of the prominent use of m above to mean something else.
...
\item Move $B_2$ in between $A_2$ and $A_1$.
This can be done in $m^2$ moves.
...
\item Move $B_2$ to its place in $R$.
	This takes $m^2$ moves~\todo{CW: I'm still confused by what it means to ``move'' $B_2$. I'll comment on slack}.
...
Let $A|B$ be the split under consideration and $T_A$, $T_B$, $R_A$, $R_B$---corresponding restriction of trees $T$ and $R$.
...
The main ingredient of the proof is that mixing $T_A$ and $T_B$ together and then separating them into $R_A$ and $R_B$ takes more steps than just convert $T_A$ into $R_A$.
...
We first bound the number of neighbors of a tree $x$ in $\dtt_2$ graph that are closer than $x$ to a given tree $y$.
...
This will be useful in future studies analagous to~\autocite{Whidden2015-es}.
}
...
%EM Sorry, but isn't this next sentence the result of Theorem 8, which is not yet completely proven?
...
Those symmetries can be employed to consider only a single representative from equivalent pairs of trees, such as by using tanglegrams \autocite{Matsen2015-fn} to reduce the number of pairs of trees for computing the curvature for all pairs of trees \autocite{Whidden2015-es}.
...
A tree is hence given by a sequence of sets of taxa, and this obtained representation is unique.
...
\vskip10pt
%EM I would think that it would give a boost to the paper for SODA to show that there are many unsolved problems in the area that other researchers may be interested in working on. So is it worth including these, or are they not mature enough or do you want to keep them secret? In any case, a "future directions" section, with a mention in the abstract/introduction, would be good IMHO.
...
The boundary $\partial S$ of $S$ is the set of edges ``pointing out from $S$'': $\partial S = \{(v,w) \in E \mid v \in S ~\&~ w \notin S\}$.
...
This proposition implies for phylogenetic graphs that the isoperimetric number is asymptotically the smallest possible, which can be interpreted as that the graphs have the tightest possible bottlenecks.
