...
\todo[inline]{
I suggest reorganizing into 1. a section on 1-neighborhood sizes, 2. a section on larger neighborhood sizes and diameter, 3. degreeBounds, intersecNeighb, and max good neighbors, 4. curvature.
...
\todo[inline]{To submit to a CS journal, we'll need to define what trees and topologies are.}
...
This corresponds to situations where all sampling was done at the same time.
Many geometric and algorithmic properties differ substantially between ultrametric and non-ultrametric trees, as shown by \textcite{Gavryushkin2014-bw}.
Hence, we consider the case of ultrametric trees separately, and indeed establish further differences.

\todo[inline]{I feel that it would be clearest to start with the "from scratch" discrete definition in this technical introduction and then describe how this relates to t and tau space, but happy to discuss.}
...
A {\em ranked topology} is a phylogenetic tree equipped with a total order on its nodes.
...
We define the {\em ranked topology of tree} $T$ to be the (directed) graph induced by the topology of $T$ such that the set of nodes of this graph is totally ordered with respect to their time order in $T$.
...
One might suggest that geometrically and graph-theoretically $\mdts$ is similar to NNI space on (rooted) tree topologies, to ranked NNI space, and to $\mdts$ space on ultrametric trees. \todo{This sentence is hard to parse!}
That is so indeed.
...
The $\mdts$ space with all intercoalescent intervals of length one is denoted by $\rnni$ and called {\em ranked NNI} space; if this space is defined on ultrametric trees we will denote it $\rnniu$.
...
\proof
The work-in-progress proof is on \url{https://github.com/gavruskin/curvatureNNI/tree/splitTheoProof} branch.
\endproof
