...
We prove the theorem in $\rnniu$ and note that for the other spaces, it follows similarly.
\todo{Explain this, as we don't have an upper bound for the diameter of the other spaces.}

Let $T = T_0, \ldots, T_k = R$ be an $\rnniu$ path (denote this path by $\gamma$) of length $k = d_\rnniu(T, R)$ such that both $T$ and $R$ have a split $A|B$ and there exists $r$ such that $T_r$ does not have the split $A|B$.
Denote the restriction of $T$ to the set of taxa $A$ by $T_A$, and define $T_B$, $R_A$, and $R_B$ similarly.
...
We say that two taxa $a_1$, $a_2$ from $A$ are {\em siblings} for $\gamma$ if they have either an identical parent or adjacent parents in $T_i$ for all $i = 0,\ldots,k$.
We define the notion of siblings for taxa in $B$ in the same way.
Since siblings are never separated along $\gamma$, we can merge them and that path that is induced by $\gamma$ between the trees with merged siblings remains a shortest $\rnniu$ path.
We proceed formally as follows.
For every pair of siblings $a_1$ $a_2$ from $A$, we replace them by $a_1$ and merge their parents.
Repeat this process for all siblings in $A$ and $B$ in both trees $T$ and $R$.
Since the path induced by $\gamma$ induce a shortest path between trees without siblings, we assume from now on that trees $T$ and $R$ have no siblings.

Since neither $T$ nor $R$ have siblings, there exist numbers $r$ and $q$ such that
\[
d(T, T_r) \le 1 + \ldots, + s = \frac 12 s (s+1),
\]
where $s$ is the number of internal nodes in $T_A$.
By the same reason, $d(R_r, R) \le \frac 12 \ell (\ell + 1)$, where $\ell$ is the number of internal modes in $R_B$.
Without loss of generality, can assume that $s = \ell = \frac n2 - 1$.
Hence,
\[
d(T,R) \ge \frac{n^2}{8} - \frac n4 + d(T_r, R_q) + \frac{n^2}{8} - \frac n4 = \frac{n^2}{4} - \frac n2 + d(T_r, R_q).
\]
Furthermore, $d(T_r, R_q) \ge \min(d(T_A, R_A), d(T_B, R_B))$.
Corollary~\ref{diameterUpperBoundCoro} implies that
\[
\frac{n^2}{4} - \frac n2 > \max(d(T_A, R_A), d(T_B, R_B)).
\]
Thus, $d(T, R) > \min(d(T_A, R_A), d(T_B, R_B)) + \max(d(T_A, R_A), d(T_B, R_B)) = d(T_A, R_A) + d(T_B, R_B)$.
...
\todo{This is true only when $T_A$ and $T_B$ are rank-separated.
