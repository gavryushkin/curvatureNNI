...
Since siblings are never separated along $\gamma$, we can merge them and the path that is induced by $\gamma$ between the trees with merged siblings remains a shortest $\rnniu$ path.
We proceed formally as follows.
For every pair of siblings $a_1, a_2$ from $A$, we replace them by $a_1$ and merge their parents ($a_1$ can be chosen arbitrarily from the pair of siblings).
Repeat this process for all siblings in $A$ and $B$ in both trees $T$ and $R$.
Since the path induced by $\gamma$ induces a shortest path between trees without siblings, we assume from now on that trees $T$ and $R$ have no siblings.
