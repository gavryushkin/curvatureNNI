...
For simplicity, we use the generic identifier $\dtt$ to refer to a $\dtt_m$ with an arbitrary $m \geq 1$.
...
It is not hard to see that the number of trees is $(n-1)! \cdot n! \cdot n!$ in $\dtt$, $(n-1)! \cdot n!$ in $\dttu$, $\frac{(n-1)!n!n!}{2^{n-1}}$ in $\rnni$, $\frac{(n-1)!n!}{2^{n-1}}$ in $\rnniu$, and $(2n - 3)!!$ in $\nni$.
...
We claim that tree labels $\alpha,\beta,\gamma$ for the vertices is enough.
We must redefine the notion of readiness to verify this claim.
We say that a pair of consecutive nodes is \emph{ready} if the label of the bottom node is $\alpha$ and the label of the top node is either $\beta$ or $\gamma$.
If two consecutive nodes are ready and not connected by an edge in the tree, then only the rank move is possible.
If they are, the type of the move is determined by the label of the top node.
...
\begin{corollary}
Bounds of the diameters from below...
\todo{}
\end{corollary}
...
We assume that $|A_1| = |A_2| = |B_1| = |B_2| = \frac{n-1}{4}$ and denote this number by $p$.
...
%EM Another pedantic point: what you mean is that this can be done in at most xxx moves. It's not obvious that it can be done in exactly this number, is it? If so, I think it's worth adding a collection of "at most"s to satisfy reviewers like me.
This can be done in $3p^2$ moves.

\item Move the caterpillar tree on $A_2$ to the other side of the root.
This can be done in $\frac12 p^2 + \frac12 p$ moves.
...
This can be done in $2p^2$ moves.

\item Swap the caterpillars on $A_2$ and $B_1$.
This can be done in $p^2$ moves.
...
\item Convert the tree on $A_1 \cup B_2$ to that in $R$.
%EM The exact rankings won't be quite right, will they? Mr. Pedantic suggests:
%EM Convert the trees on $A_1$ and $B_2$ to the corresponding subtrees in $R$ in terms of topology and relative ranking of nodes within the subtrees.
This takes $2f(n/4)$ moves.

\item Translate the tree on $B_2$ down so that the tree on $A_1 \cup B_2$ coincides with that in $R$.
%EM If you agree with Mr. Pedantic above, then perhaps you might use:
%EM Translate the tree on $B_2$ down so that the tree on $A_1 \cup B_2$ matches the corresponding subtree in $R$ in terms of topology and relative ranking of nodes.
This takes $p^2$ moves.

\item Merge the tree on $A_2$ with the tree on $B_2$ so that the tree on $A_1 \cup A_2 \cup B_2$ coincides with that in $R$.
This takes $2p^2$ moves.

\item Merge the tree on $B_1$ with the tree on $A_1$ so that the tree coincides with $R$.
This takes $3p^2$ moves.
\end{enumerate}

In total, we have a recursive equation $f(n) = 14p^2 + p + 2 f(n/4)$.\\
...
Finally, note that the assumption $|A_1| = |A_2| = |B_1| = |B_2| = \frac{n-1}{4}$ does not increase the distance between $T$ and $R$.
...
%EM Second use of Finally in this paragraph.
