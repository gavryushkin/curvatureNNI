...
\proof
We prove the theorem in $\rnniu$.
The theorem will follow similarly in the other three spaces.
The proof employs the technique from \autocite{Sleator1992-bp} for counting paths in a graph.

In terms of the terminology of \textcite{Sleator1992-bp}, it is enough to note the following.
Three labels $1,2,3$ are enough.
Indeed, our configurations are pairs of nodes of consecutive ranks.
We call them the {\em top} and the {\em bottom} node of the configuration according to their ranks.
A configuration is ready if the label of the bottom node is $1$ and the label of the top node is either $2$ or $3$.
The three productions of our grammar correspond to two possible $\nni$ moves and one possible rank move.
If two consecutive nodes are not connected by an edge and the configuration is ready, only the rank move is possible.
If they are, the type of the rank move is determined by the label of the top node.
Hence, the right-hand sides of all productions have two nodes.
In total, $n-1$ internal nodes result in $3^{n-1}$ possible labellings of the initial tree, plus every move creates two new nodes, each of which can have three possible labellings, hence the number of canonical paths is $3^{n-1} \cdot 3^{2r}$.

To make this paper self-containing, we provide a detailed proof:
\todo{Do we need this?} 

%Let $T = T_1,\ldots, T_r$ be a path of length $r$ in $\rnniu$ (hence, $d_\rnniu(T, T_r) \le r$).
%We assume that the path is {\em canonical} meaning that if several moves to be applied on disjoint intervals, they are applied in the order of the ranks of nodes involved.
%Since, every path between a pair of trees can be converted to a unique canonical path, the number of canonical paths from tree $T$ of length up to $r$ bounds the number of trees withing distance $r$ from $T$ from above.

%We now fix a tree $T$ an encode every canonical of length $r$ starting in $T$ by a ternary sequence of length $n+2m-1$.
\endproof
