...
By a {\em time-tree} (see Figure~\ref{timeTree.eps}) we mean a rooted phylogenetic tree with an absolute time associated with every node of the tree: for internal nodes these are (typically estimated) divergence times and leaves are (typically known) sampling times.
...
Assuming that the length of all intercoalescent intervals is either $1$ or $2$, the outer trees are all possible neighbors in $\dtt$ of the tree in the middle corresponding to interval $I$.}
\end{figure}
