...
\todo{This also improves the best known bound for NNI.
Add NNI to the statement?}
...
\todo{This could result in deleting all taxa, e.g.\ if the whole $B$ goes under $A$.
...
\todo{This is the main ingredient of the proof, which so far remains illusive.}
Thus, we assume that $T$ and $R$ do not have taxa with identical mixing contribution.
\todo{Inessential nodes are exactly those with mixing contribution $0$.
Should we unify the two steps?}
...
\noindent {\bf Main ingredient outline:}

First prove:\\
{\em If there exists a shortest splitless path $\alpha$, then there exists a splitless path $\gamma$ such that two nodes from $T_A$ and $T_B$ are never separated by $c$ ranks, where $c$ is a constant independent from $n$.}

Then apply the argument above that pretty much says that:\\
{\em When $n$ grows, the gap between ranks of nodes from $T_A$ (and $T_B$) must grow.}

These two would imply that starting from some $n$, there exists no shortest splitless path.
\endproof
