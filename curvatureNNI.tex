...
\begin{lemma}
Let $T$, $R$ be a pair of trees sharing a split $A \mid B$ such that there exists a shortest path $\gamma$ between $T$ and $R$ and a tree $S$ on $\gamma$ such that $S$ does not have $A \mid B$.
Then there exist two disjoint sets of taxa $A'$, $B'$, trees $T'$, $R'$ on $A' \cup B'$, a shortest path $\gamma'$ from $T'$ to $R'$, and a number $n'$ such that $|A'| = |B'| = n'$, both $T'$ and $R'$ have the split $A'|B'$and
\[
d(\gamma', A'|B') \ge \frac{n'^2}{8} - \frac{n'}{4},
\]
where $d(\gamma', A'|B') = \max\{d(T, A'|B') : T\in\gamma'\}$ and $d(T, A'|B') = \min\{d(T, S) : S \mbox{ has } A'|B'\}$.
\end{lemma}

It follows from this lemma that $\gamma'$ contains a tree $S'$ such that
\[
d(T',R') \ge 2d(T',S') + d(T'_{A'}, R'_{A'}) \ge \frac{n'^2}{4} - \frac{n'}{2} + d(T'_{A'}, R'_{A'}),
\]
We now apply Corollary~\ref{diameterUpperBoundCoro} and get
\[
d(T',R') > d(T'_{B'}, R'_{B'}) + d(T'_{A'}, R'_{A'}).
\]
This is however a contradiction because $d(T',R') \le d(T'_{A'}, R'_{A'}) + d(T'_{B'}, R'_{B'})$ by converting $T'_{A'}$ to $R'_{A'}$ and then independently $T'_{B'}$ to $R'_{B'}$ thus preserving the split along the path.
