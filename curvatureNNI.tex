...
Then for $n \ge 4$,
\[
\Delta(\rnniu) \le \frac{13}{14}n^2 - \frac{11}{4}n - \frac{9}{16}.
\]
\end{lemma}
...
In total we have a recursive equation $f(n) = 13m^2 + m + 2 f(n/4)$.\\
The solution to this equation is
\[
f(n) = \frac{13}{14}n^2 - \frac{11}{4}n - \frac{9}{16}.
\]
...
$\Delta(\rnniu) < n^2 - \frac 14$ for all $n$.
\end{corollary}
...
Suppose there exists a pair of trees $T,R$ sharing a split $A \mid B$ such that there exists a shortest path between $T$ and $R$ and a tree $T'$ on this path such that $T'$ does not have $A \mid B$.
Let $T_A$ be the subtree of $T$ on taxa $A$ and $T_B$, $R_A$, $R_B$ are defined analogously.
We choose $T'$ to be a tree on the path from $T$ to $R$ that minimizes $\{|X| : X \mid Y \mbox{ is a split in $T'$ and } X \subseteq A\}$.
Denote this number by $\sigma$.
Without loss of generality, we can assume that $|A| = |B| = \frac n2$ and $\sigma = 1$.
Indeed, this can be achieved by considering an appropriate subpath between appropriate subtrees of $T$ and $R$ and collapsing parts of the tree that are moved together (or not moved at all) into a taxon.

First, we assume that all nodes of $T_A$ ($R_A$) have lower ranks than those in $T_B$ ($R_B$).
Since $|A| = |B| = \frac n2$ and $\sigma = 1$,
\[
d(T,T') \ge \frac{n^2}{8} - \frac 18.
\]
Furthermore,
\[
d(T,R) \ge 2d(T,T') + d(T_A, R_A) \ge \frac{n^2}{4} - \frac 14 + d(T_A, R_A),
\]
We now apply Corollary~\ref{diameterUpperBoundCoro} and get
\[
d(T,R) > d(T_B, R_B) + d(T_A, R_A).
\]
This is however a contradiction because under our assumption about ranks of nodes in $T$ and $R$, we have that $d(T,R) \le d(T_A, R_A) + d(T_B, R_B)$ by converting $T_A$ to $R_A$ and then independently $T_B$ to $R_B$ thus preserving the split along the path.

The case when the assumption on ranks of nodes in $T$ and $R$ does not hold follows similarly by noting that the number of extra rank moves required for ``merging'' $T_A$ and $T_B$ is not less than the number or rank moves required to ``separate'' the nodes in $T_A$ from the nodes $T_B$ to then transform them independently.
\endproof
