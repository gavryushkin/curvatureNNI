...
\todo{EM: My preference would be to only use "space" for collections of trees with a continuous component.}
...
Furthermore, $\dtt$ can be seen as a subset of the integer lattice on each orthant in the BHV space \autocite{}.
The edge relation in $\dtt$ is then induced by the BHV distance being less than some fixed number.
...
The generalization of this statement to an arbitrary number of leaves implies the convexity.
...
We first bound the number of neighbors of a tree $x$ in $\dtt_2$-space that are closer than $x$ to a given tree $y$.
...
\begin{proof}
We first prove the statement for $\dtt_2$ space.
...
\begin{lemma}
The following inequalities are satisfied in both $\dtt_2$ and $\dtt$ spaces.
\[
