...
function.\todo{Make this formal if the NNI-conjecture is not proven.} 
But since we are mostly interested in this paper in the spaces under
consideration, we will not 
\todo{Should we?}
include the statement in general terms of metric 
spaces. However, we prove a much stronger property for $\dts$ space: 
...
\proof
\todo{Should follow from the NNI conjecture.}
\endproof
...
At most $d(x,y)$ bipartitions differ between $x$ and $y$. 
It is never optimal to break a common bipartition with an NNI operation.
There are at most $2d(x,y)$ possible NNI operations that can be applied to $x$ that operate on a unique bipartition.
There are at most $d(x,y)$ rank changes that can be applied to move $x$ closer to $y$.
There are at most $d(x,y)$ length changes that can be applied to move $x$ closer to $y$.
Therefore, there are at most $4d(x,y)$ trees $u \in N(x)$ such that $d(u, y) \le d(x, y)$. 
\end{proof}
