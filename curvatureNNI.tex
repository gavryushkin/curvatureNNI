...
The last ten years have seen an explosion of methods for inferring demographic model parameters from sequence data by sampling phylogenetic trees \autocite{Kuhner1995-mj,Kuhner1998-tq,Kuhner2000-af,Beerli2001-sc,Kuhner2006-vx,Drummond2005-ks,Drummond2006-oa,Minin2008-wz}.
Dates defined in phylogenetic terms, such as time to a most recent common ancestor of human HIV group M viruses \autocite{Worobey2008-rt,Baele2013-op}, are an important such parameter.
...
Posterior distributions on time trees, as well as classical phylogenetic trees equipped with branch lengths quantifying the amount of molecular substitution along a branch are estimated using Markov chain Monte Carlo \autocite{Mau1997-sq,Yang1997-gv,Drummond2006-oa}.
Although Markov chain Monte Carlo (MCMC) is guaranteed to sample the posterior given an infinite run time, it is important to understand mixing properties of the chain, which determine sampling properties for a finite time run.
...
MCMC moves between classical phylogenetic trees are typically defined in terms of this discretization, such as the \emph{subtree prune and regraft} (SPR) move, that cuts a subtree off and reattaches it at another location.
...
The graph build on such trees provides a further refined discretisation of the space of time trees.
...
We will study Ricci-Ollivier curvature \autocite{Ollivier2009-cj} of Markov chains on the graphs, which quantifies to what extent random walking brings points together.
...
Although most of the results in this paper can be generalised to the class of sampled ancestor trees \autocite{Gavryushkina2014-xd}, we do not make this generalisation, as currently very few methods of sampling the space of sampled ancestor trees are known \autocite{Gavryushkina2015-vq} and the geometry of the space is poorly understood \autocite{Gavryushkin2014-bw}.

As described in the introduction, we will discretize the geometry necessary for these investigations is yet to be developed to the necessary level \autocite{Gavryushkin2014-bw}.
...
Furthermore, many geometric and algorithmic properties vary substantially for ultrametric and non-ultrametric trees, as shown by \textcite{Gavryushkin2014-bw}.
...
An important notion for the study of mixing properties of Markov chains on graphs is the notions of distance.
A natural distance on a graph is given by the length of a shortest path between two vertices.
Hence, finding this shortest path is an important computational problem in the study of efficiency properties of the space.
It is well-known that this problem is NP-hard in $\nni$ space \autocite{Dasgupta2000-xa}.
...
Although we approach this problem by solving it in a partial case, a complete answer is yet to be discovered.
...
We start with the following lemma that bounds the sizes of one-neighbourhoods if various discretisations of the space of time trees.
The lemma is of the the most important tools for estimating the curvature of various random walks later in the paper.
