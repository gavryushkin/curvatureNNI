...
The last ten years have seen an explosion of methods for inferring demographic model parameters from sequence data by sampling phylogenetic trees \cite{Kuhner1995-mj,Kuhner1998-tq,Kuhner2000-af,Beerli2001-sc,Kuhner2006-vx,Drummond2005-ks,Drummond2006-oa,Minin2008-wz}.
Dates defined in phylogenetic terms, such as time to a most recent common ancestor of human HIV group M viruses \cite{Worobey2008-rt,Baele2013-op}, are an important such parameter.
...
Posterior distributions on time trees, as well as classical phylogenetic trees equipped with branch lengths quantifying the amount of molecular substitution along a branch are estimated using Markov chain Monte Carlo \cite{Mau1997-sq,Yang1997-gv,Drummond2006-oa}.
...
\caption{A time tree on $6$ taxa.}
\label{T5.pdf}
\end{figure}

The mixing properties of phylogenetic MCMC have, in the classical unrooted case, been a major area of research from both the theoretical perspective of mixing time bounds~\cite{Mossel2005-ly,Mossel2006-fo,Stefankovic2011-hu,spade2014note} and the practical perspective of performance on real data~\cite{beiko2006searching,Ronquist2006-fv,lakner2008efficiency,Whidden2015-yi}.
...
MCMC moves between classical phylogenetic trees are typically defined in terms of this discretization, such as the \emph{subtree prune regraft} (SPR) move, that cuts a subtree off and reattaches it at another location.
On the other hand, moves between time trees are also defined in terms of their timing information.
For example, \cite{Hohna2008-vl} show that SPR-like moves that reattach subtrees at the same divergence time are more effective than ones that do not.
...
For example, by retaining the order of internal nodes forward in time one obtains a so-called \emph{ranked tree}, cite{}.
A sequence of time trees sampled using MCMC projects to a collection of movements on a graph in which each vertex is a discretized time tree and each edge is a discretized version of a MCMC move on time trees.
...
In particular, we focus on ... these graphs...
We will study Ricci-Ollivier curvature~\cite{Ollivier2009-cj} of Markov chains on the graphs, which quantifies to what extent random walking brings points together.
...
As described in the introduction, we will discretize the geometry necessary for these investigations is yet to be developed to the necessary level~\cite{Gavryushkin2014-bw}.
...
One of the central properties of Ricci-Ollivier curvature is that the curvature is a local property~\cite{Ollivier2009-cj}.
