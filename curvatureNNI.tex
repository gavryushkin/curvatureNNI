...
\address{Department of Biosystems Science and Engineering, ETH Z\"urich, 4058 Basel}
...
Furthermore, we design an efficient algorithm for constructing these graphs.
...
The node $v$ cannot be destroyed as a bottom node either because the node $w$ right on top of $v$ in the running tree at a later stage $s$ of the derivation can be either a bottom node or a node that has to be preserved, in both of these cases node $w$ must be marked with an $a$.
...
The history of research into computational complexity of phylogenetic graphs, and particularly the $\nni$ graph, is very rich and exciting, with a number of erroneous results being published over the 25 years it took to settle the complexity.
...
The latter claims were mainly based on the so-called Split Theorem that states the following.
...
These claims have been refuted by \textcite{li1996some}, who proved that the Split Theorem does not hold in the $\nni$ graph.

Hence, a natural question that would contribute to understanding of Problem~\ref{complexityProblem} is whether or not the Split Theorem holds in the $\rnniu$ graph and other time-tree graphs.
...
\begin{conjecture}\label{splitConj}
The following is true in all graphs $\dtt$, $\dttu$, $\rnni$, and $\rnniu$, but not in $\nni$.
If a partition of leaves given by an edge presents in two trees $T$ and $R$ then the partition presents in every tree on every shortest path between $T$ and $R$.
\end{conjecture}


\subsection{Computing the distance}

In this section, we bound the number of neighbors of a tree $x$ in the discrete time-tree graphs that are closer than $x$ to a given tree $y$, \textbf{under the assumption that Conjecture~\ref{splitConj} holds}.\footnote{
...
Furthermore, the result provides a further insight into possible approaches to Problem~\ref{complexityProblem}.
}

\begin{theorem}
\label{max_good_neighbors}
Assume that Conjecture~\ref{splitConj} holds.
...
Now, Conjecture~\ref{splitConj} implies that no minimal length path from $x$ to $y$ will replace a bipartition that is common to $x$ and $y$.
