...
\usepackage{graphicx}
\usepackage{etoolbox}
\usepackage[style=alphabetic,ibidtracker=false,uniquename=false]{biblatex}
...
\title{The combinatorics of discrete time-trees: theory and open problems}
...
\thanks{AG was supported by the Marsden Fund of New Zealand through the Centre for Computational Evolution.}


\begin{abstract}
A \emph{time-tree} is a rooted phylogenetic tree with all internal nodes equipped with absolute divergence dates.
We introduce and study a hierarchy of discrete approximations of the space of time-trees from the graph-theoretic and algorithmic point of view.
...
We study the basic graph-theoretic questions such as the size of neighborhoods, the diameter upper and lower bound, the problem of finding shortest paths, and similar, for the introduced graphs.
We settle many of these questions by applying the concept of graph grammars introduced by Sleator, Tarjan, and Thurston to our graphs.
Furthermore, we design an efficient heuristic for generating these graphs.
...
We discuss the directions that are most valuable for phylogenetic applications and give a list of open problems that we think are of prominent importance for those applications.
\end{abstract}
...
This paper builds a foundation for a mathematical understanding of time trees, such as understanding mixing properties of MCMC thereupon.
...
This research has focused on mixing over the set of discrete phylogenetic tree graph structures, and mixing over these discrete structures is the primary obstruction to MCMC convergence.
...
\begin{problem}\label{complexityProblem}
...
\begin{lemma}[see \autocite{Semple2003-nj}]\label{spaceSizes}
...
The rest follow from the $\nni$ case.
\endproof

We continue with the following bounds on the sizes of $1$-neighborhoods of various discretizations of the space of time-trees.
\iftoggle{curvatureON}{
...
We use two node-labels (\textcite{Sleator1992-bp} call these "labels of vertices") to identify these two roles.
Furthermore, to distinguish which of the two possible $\nni$ moves is applied, we employ another (third) node-label.
We claim that three node-labels $a, b, c$ is enough.
...
Furthermore, if we want to indicate that a pair of consecutive nodes is never destroyed, we give them identical node-labels.
...
%TODO: Add this for the full version?
...
The following algorithm to compute the $\rnni$ graph on $n$ leaves, which is a natural extension of an algorithm from \textcite{Whidden2015-es}.
...
Problem~\ref{complexityProblem} mentioned in the beginning of previous section asks about the complexity of computing the distance in discrete time-tree graphs.
This problem is the primary obstacle for actual biological applications of the introduced graphs.
The history of research into computational complexity of phylogenetic graphs, and particularly the $\nni$ graph, is very reach, with a number of erroneous results being published.
See \autocite{Dasgupta2000-xa} for a detailed discussion of those publications, some of which claimed that $\rnni$ distance is NP-hard while others claiming that $\rnni$ distance is decidable in polynomial time.
The latter claims were mainly based on the so-called Split Theorem that claims that if a partition of leaves given by an edge is shared between trees then there exists a shortest $\nni$ path between the trees such that every tree on the path maintains the partition.
These claims have been refuted by \textcite{li1996some}.

Hence, a natural question that would contribute to understanding Problem~\ref{complexityProblem} is whether or not the Split Theorem holds in $\rnniu$.
...
We first bound the number of neighbors of a tree $x$ in the $\dtt_2$ graph that are closer than $x$ to a given tree $y$.
