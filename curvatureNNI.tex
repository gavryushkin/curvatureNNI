...
\item The trees have topologies which differ by one NNI move and
\begin{itemize}
\item the NNI move is done by sending the length of one intercoalescent interval down to zero and resolving the multifurcation in one of the two possible ways, 
\item all nodes that are not involved in the NNI move have identical ranks, 
\item all intercoalescent intervals that are not involved in the NNI move have identical lengths,
\item the two intervals that are involved in the NNI move (the one that was there before the move and the one that appeared after the move) both have the minimal possible length.
\end{itemize}
\end{enumerate}
