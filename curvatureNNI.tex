...
$\Delta(\rnniu) < n^2 - n - \frac 14$ for all $n$.
\end{corollary}
...
Indeed, otherwise there would exist a $p \in \{1,\ldots,s\}$ such that the number of taxa $a \in A$ which parents' ranks are never above $p$ along $\gamma$ is greater than the number of taxa $b \in B$ which parents' ranks are never below or equal to $p$.
...
Since $d(T,R) = d(T,T_r) + d(T_r,T_q) + d(T_q,R)$ and $s + \ell = n-1$, we have that
\[
d(T,R) \ge \frac 12 s^2 + \frac 12 s + d(T_r, T_q) + \frac 12 (n-1-s)^2 + \frac 12 (n-1-s).
\]
The latter is minimized when $s = \frac{n-1}{2}$.
Hence,
\[
d(T,R) \ge \frac{n^2}{4} - \frac n2 - \frac 14 + d(T_r, T_q).
\]
Furthermore, $d(T_r, T_q) \ge \max(d(T_A, R_A), d(T_B, R_B))$.
Corollary~\ref{diameterUpperBoundCoro} implies that
\[
\frac{n^2}{4} - \frac n2 - \frac 14> \min(d(T_A, R_A), d(T_B, R_B)).
\]
