...
For example, \autocite{Hohna2008-vl} show that SPR-like moves that reattach subtrees at the same divergence time are more effective than ones that do not.
...
%EM It seems to me that the following sentence is optional.
...
the complexity cannot be derived from that of $\nni$ distance.
...
We now bound the number of neighbors of a tree $x$ in the discrete time-tree graphs that are closer than $x$ to a given tree $y$, \textbf{under the assumption that Conjecture~\ref{conjSplit} holds}.\footnote{
